...
- Compose the configured primary communication/chat harness exactly as before. When role routing is configured, build one additional supervisor per unique routed provider and attach them through the service role-harness runtime; roles that select the primary provider or omit a route continue to use the primary fallback. Additional routed providers share workspace execution policy but do not inherit the primary provider's model, reasoning effort, or service mode.
