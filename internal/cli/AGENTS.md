...
- Compose routed and unrouted configurations through the same harness.Runtime and RuntimeSpec: one Supervisor per normalized unique provider, chat selected by harness.name, and unspecified roles falling back to chat. The chat provider retains model, reasoning/legacy effort, and service mode; distinct routed providers use provider inference defaults while sharing workspace, sandbox, approval, and network policy. Preserve global custom ACP command/argument semantics and validation. Live settings reconcile through Runtime without separate primary or routed Start/Close paths. Server shutdown closes the service's single lifecycle owner; orchestration starts independently and gates each phase on its routed provider's availability, with broadcast readiness waking rechecks.
