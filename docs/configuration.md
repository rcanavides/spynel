...
- `enabled`, `interval_seconds`, and `max_parallel` control ordinary route scanning and dispatch. While every agent still shares one workspace checkout, route-scanned workflow execution is temporarily serialized to a single writer regardless of `max_parallel`: `max_parallel` remains the general configured capacity ceiling, and a workflow parked on the writer slot never consumes a capacity slot. The single-writer gate spans the provider turn through the first non-continuing terminal event and the post-terminal pipeline. Notification and heartbeat ordinary agent turns are unaffected.
- `workspace_isolation` selects the execution mode for new task claims: `shared` (the default) keeps the single shared checkout, and `git-worktree` gives every isolated task implementation and review launch its own detached locked Git worktree. It applies live to new claims only; a running lineage keeps its recorded mode. Isolated claims require a passing Git preflight (git installed, at least Git 2.25, a non-bare repository, an attached root target, and an untracked `.spynel`) and fail closed without falling back to shared mode. See “Isolated workflow execution” in the architecture documentation for the full evidence chain; isolated workspaces are workflow integrity, not operating-system sandbox security.
- `checks` is an optional YAML-only list (there is no command setting) of system checks Spynel itself runs against an isolated launch's captured result, newest first in the configured order. Each entry has an `id` matching `[a-z0-9][a-z0-9._-]{0,63}`, a required shell-free `command` with optional `args`, and a `timeout` between one second and two hours defaulting to `10m`. At most sixteen entries with unique ids are accepted. The exact set is frozen per launch when it is created and recorded in the launch evidence; later configuration changes affect only new launches, and recovery reruns only checks whose evidence is missing.
...
`workspace.cleanup_retention_days` defaults to `30` for new and existing workspaces and applies live. The elected primary runs cleanup every eight hours. `/cleanup [days]` runs the identical operation manually and defaults to `7`; values must be whole days from 1 through 36500. History, job, and task eligibility is strict: an item timestamped exactly at the cutoff is retained, and only items older than the cutoff are processed. Failed, rejected, superseded, and reviewer isolated workspaces use the same configured days as a positive duration and become eligible when their evidence age reaches that duration; integrated and not-applicable workspaces remain eligible on the next pass.
